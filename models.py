...
class HospitalManagement:
    ...
    def doctor_menu(self):
        ...
        if sub_option == 1:
            doctor_id = click.prompt("Enter NHS ID for the doctor you want to search", type=int)
            print(Doctor.doctor_details(doctor_id))
        elif sub_option == 2:
            doctor_id = click.prompt("Enter NHS ID for doctor to get the nurses under the doctor", type=int)
            Doctor.get_nurses(doctor_id)
        elif sub_option == 3:
            doctor_id = click.prompt("Enter NHS ID for doctor to retrieve the patients for the doctor")
            Doctor.get_patients(doctor_id)
        else:
            print("Invalid option selected!")
    def nurse_menu(self):
        sub_option = click.prompt("What would you like to do?\n1.Get nurse details using NHS ID\n2. See doctor a particular nurse reports to.\n3. View patients assigned to a specific nurse\nSelect an option", type=int)

        if sub_option == 1:
            nurse_id = click.prompt("Enter NHS ID for the nurse to get the full details", type=int)
            print(Nurse.get_details(nurse_id))
        elif sub_option == 2:
            nurse_id = click.prompt("Enter NHS ID for nurse to get the doctor assigned the nurse", type=int)
            print(Nurse.get_doctor(nurse_id))
        elif sub_option == 3:
            nurse_id = click.prompt("Enter NHS ID for nurse to see patients assigned to a particular nurse", type=int)
